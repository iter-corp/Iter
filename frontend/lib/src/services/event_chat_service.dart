...
class EventChatService {
  ...
  Stream<List<EventChatSummary>> streamMyEventChats(String uid) async* {
    ...
    try {
      yield* _db
        ...
        for (final doc in docs) {
          ...
          final unread = (lastTime != null &&
                  lastSeen != null &&
                  lastTime.isAfter(lastSeen))
              ? 1
      ...
      if (e.code == 'failed-precondition') {
        // Index may still be building. Keep UI alive and retry on next
        // provider refresh/app restart.
        yield const <EventChatSummary>[];
        return;
      }
      rethrow;
    }
