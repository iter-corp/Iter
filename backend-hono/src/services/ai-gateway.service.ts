...
export class AiGatewayService {
   ...
   */
  private async getOrderedKeys(): Promise<Array<{ id: string; provider: string; key: string }>> {
    ...
    const envList: Array<{ id: string; provider: string; key: string }> = [];
    if (env.GEMINI_API_KEY) envList.push({ id: 'env_gemini', provider: 'gemini', key: env.GEMINI_API_KEY });
    if (env.OPENAI_API_KEY) envList.push({ id: 'env_openai', provider: 'openai', key: env.OPENAI_API_KEY });
    if (env.ANTHROPIC_API_KEY) envList.push({ id: 'env_claude', provider: 'claude', key: env.ANTHROPIC_API_KEY });
    if (env.AZURE_TRANSLATOR_KEY) envList.push({ id: 'env_azure', provider: 'azure', key: env.AZURE_TRANSLATOR_KEY });
    return envList;
  }
  ...
  private async callGemini(text: string, sourceLang: string, targetLang: string, apiKey: string): Promise<string> {
  ...
  private async callOpenAi(text: string, sourceLang: string, targetLang: string, apiKey: string): Promise<string> {
  ...
  private async callClaude(text: string, sourceLang: string, targetLang: string, apiKey: string): Promise<string> {
  ...
  private async callAzure(text: string, sourceLang: string, targetLang: string, apiKey: string): Promise<string> {
