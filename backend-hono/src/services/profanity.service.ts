...
export async function getProfanityList(): Promise<Set<string>> {
  const now = Date.now();
...
export async function findProfanityMatches(text: string): Promise<string[]> {
...
export function maskProfanity(text: string, matches: string[]): string {
  let masked = text;
