...
userRoutes.patch('/me', requireAuth, zValidator('json', updateProfileSchema), async (c) => {
  ...
  if (body.appIntroSeen !== undefined) patch.appIntroSeen = body.appIntroSeen;
  if (body.profession !== undefined) patch.profession = body.profession;
  if (body.field !== undefined) patch.field = body.field;
  if (body.academicLevel !== undefined) patch.academicLevel = body.academicLevel;
...
userRoutes.delete('/:uid/follow', requireAuth, async (c) => {
  ...
  if (existing) {
    ...
    await db.delete(notifications).where(
      and(eq(notifications.targetUid, targetUid), eq(notifications.actorUid, followerUid), inArray(notifications.type, ['follow', 'follow_request']))
    );
  }
...
userRoutes.post('/:uid/block', requireAuth, async (c) => {
  ...
  await db.delete(follows).where(
    sql`(${follows.followerUid} = ${blockerUid} AND ${follows.targetUid} = ${targetUid}) OR (${follows.followerUid} = ${targetUid} AND ${follows.targetUid} = ${blockerUid})`
  );
