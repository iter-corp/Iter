...
import { chats, messages, messageReactions, users } from '../db/schema/index.js';
...
chatRoutes.get('/direct/:otherUid', requireAuth, async (c) => {
  ...
  if (!chat) {
    ...
    if (!other[0]) throw new AppError('Other user not found', 404, 'NOT_FOUND');

    const userData: Record<string, { username: string; avatarUrl?: string }> = {
...
chatRoutes.post('/:chatId/messages', requireAuth, zValidator('json', sendMessageSchema), async (c) => {
  ...
  if (!previewText) {
    if (body.imageUrl) previewText = 'Sent a photo';
    else if (body.videoUrl) previewText = 'Sent a video';
    else if (body.voiceUrl) previewText = 'Voice message';
    else if (body.stickerUrl) previewText = 'Sent a sticker';
    else if (body.fileUrl) previewText = 'Sent a file';
    else if (body.locationLat) previewText = '📍 Shared a location';
    else previewText = 'Message';
  }

  // Update chat summary & increment unread count for recipients
  const nextUnread: Record<string, number> = { ...(chat.unreadCounts || {}) };
  nextUnread[uid] = 0;
