...
import { comments, commentLikes, commentHelpfulVotes, commentReports, posts, users, notifications } from '../db/schema/index.js';
...
commentRoutes.patch('/:id', requireAuth, zValidator('json', editCommentSchema), async (c) => {
  ...
  if (existing.authorUid !== uid) throw new AppError('Permission denied', 403, 'FORBIDDEN');
