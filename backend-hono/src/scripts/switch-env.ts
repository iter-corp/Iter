...
async function main() {
  ...
  const config: EcosystemConfig = JSON.parse(fs.readFileSync(configPath, 'utf8'));
  ...
  try {
    ...
    for (const oldPrefix of oldPrefixes) {
      if (!oldPrefix || oldPrefix.length < 5) continue;
      console.log(`   🔎 Scanning & replacing: "${oldPrefix}" ➡️ "${profile.storagePublicUrl}"`);

      // A. Users (avatar_url, cover_url)
      const [uRes1]: any = await connection.execute(
        `UPDATE users SET avatar_url = REPLACE(avatar_url, ?, ?) WHERE avatar_url LIKE ?`,
        [oldPrefix, profile.storagePublicUrl, `%${oldPrefix}%`]
      );
      const [uRes2]: any = await connection.execute(
        `UPDATE users SET cover_url = REPLACE(cover_url, ?, ?) WHERE cover_url LIKE ?`,
        [oldPrefix, profile.storagePublicUrl, `%${oldPrefix}%`]
      );
      const userUpdates = (uRes1.affectedRows || 0) + (uRes2.affectedRows || 0);
      if (userUpdates > 0) console.log(`      • users: updated ${userUpdates} fields`);

      // B. Posts (author_avatar)
      const [pRes1]: any = await connection.execute(
        `UPDATE posts SET author_avatar = REPLACE(author_avatar, ?, ?) WHERE author_avatar LIKE ?`,
        [oldPrefix, profile.storagePublicUrl, `%${oldPrefix}%`]
      );
      if (pRes1.affectedRows > 0) console.log(`      • posts (author_avatar): updated ${pRes1.affectedRows} rows`);

      // Posts JSON arrays (image_urls, video_urls)
      try {
        const [postsToFix]: any = await connection.execute(
        ...
        for (const row of postsToFix) {
          let imgUrls: string[] = Array.isArray(row.image_urls) ? row.image_urls : [];
          let vidUrls: string[] = Array.isArray(row.video_urls) ? row.video_urls : [];
        ...
        console.warn('      ⚠️ Note on posts media JSON update:', (postJsonErr as any).message);
      }
      ...
      if (cRes.affectedRows > 0) console.log(`      • comments: updated ${cRes.affectedRows} rows`);

      // D. Events (cover_image_url)
      const [eRes]: any = await connection.execute(
        `UPDATE events SET cover_image_url = REPLACE(cover_image_url, ?, ?) WHERE cover_image_url LIKE ?`,
        [oldPrefix, profile.storagePublicUrl, `%${oldPrefix}%`]
      );
      if (eRes.affectedRows > 0) console.log(`      • events: updated ${eRes.affectedRows} rows`);

      // E. Stories (image_url, video_url, author_avatar)
      const [sRes1]: any = await connection.execute(
        `UPDATE stories SET image_url = REPLACE(image_url, ?, ?) WHERE image_url LIKE ?`,
        [oldPrefix, profile.storagePublicUrl, `%${oldPrefix}%`]
      );
      const [sRes2]: any = await connection.execute(
        `UPDATE stories SET video_url = REPLACE(video_url, ?, ?) WHERE video_url LIKE ?`,
        [oldPrefix, profile.storagePublicUrl, `%${oldPrefix}%`]
      );
      const [sRes3]: any = await connection.execute(
        `UPDATE stories SET author_avatar = REPLACE(author_avatar, ?, ?) WHERE author_avatar LIKE ?`,
        [oldPrefix, profile.storagePublicUrl, `%${oldPrefix}%`]
      );
      const storyUpdates = (sRes1.affectedRows || 0) + (sRes2.affectedRows || 0) + (sRes3.affectedRows || 0);
      if (storyUpdates > 0) console.log(`      • stories: updated ${storyUpdates} fields`);

      // F. Messages (image_url, video_url, voice_url)
      const [mRes1]: any = await connection.execute(
        `UPDATE messages SET image_url = REPLACE(image_url, ?, ?) WHERE image_url LIKE ?`,
        [oldPrefix, profile.storagePublicUrl, `%${oldPrefix}%`]
      );
      const [mRes2]: any = await connection.execute(
        `UPDATE messages SET video_url = REPLACE(video_url, ?, ?) WHERE video_url LIKE ?`,
        [oldPrefix, profile.storagePublicUrl, `%${oldPrefix}%`]
      );
      const [mRes3]: any = await connection.execute(
        `UPDATE messages SET voice_url = REPLACE(voice_url, ?, ?) WHERE voice_url LIKE ?`,
        [oldPrefix, profile.storagePublicUrl, `%${oldPrefix}%`]
      );
      const messageUpdates = (mRes1.affectedRows || 0) + (mRes2.affectedRows || 0) + (mRes3.affectedRows || 0);
      if (messageUpdates > 0) console.log(`      • messages: updated ${messageUpdates} fields`);
