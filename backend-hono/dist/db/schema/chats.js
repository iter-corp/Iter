...
export const chats = pgTable('chats', {
    ...
    participants: jsonb('participants').$type().default([]).notNull(),
    acceptedBy: jsonb('accepted_by').$type().default([]).notNull(),
    mutedFor: jsonb('muted_for').$type().default([]).notNull(),
    unreadCounts: jsonb('unread_counts').$type().default({}).notNull(),
    userData: jsonb('user_data').$type().default({}).notNull(),
...
export const messages = pgTable('messages', {
    ...
    seenBy: jsonb('seen_by').$type().default([]).notNull(),
    visibleToUids: jsonb('visible_to_uids').$type().default([]).notNull(),
    deletedForUids: jsonb('deleted_for_uids').$type().default([]).notNull(),
