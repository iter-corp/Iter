...
import { events, eventRegistrations, users, notifications } from '../db/schema/index.js';
...
eventRoutes.get('/', optionalAuth, async (c) => {
    ...
    if (country && country !== 'all')
        conditions.push(eq(events.locationCountry, country));
    ...
    (async () => {
        try {
            ...
            for (const u of eligibleUsers) {
                ...
                if (prefs.mode === 'off')
                    continue;
                if (prefs.types.length > 0 && !prefs.types.includes(body.eventType))
                    continue;
