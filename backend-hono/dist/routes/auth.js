...
function generateTokens(userId, email) {
    const accessToken = jwt.sign({ sub: userId, email }, env.JWT_SECRET, { expiresIn: env.JWT_ACCESS_EXPIRATION });
...
authRoutes.post('/google', zValidator('json', googleAuthSchema), async (c) => {
    ...
    if (!user) {
        ...
        const patch = {};
        if (!user.googleId)
            patch.googleId = googleSub;
        if (!user.avatarUrl && googleData.picture)
            patch.avatarUrl = googleData.picture;
