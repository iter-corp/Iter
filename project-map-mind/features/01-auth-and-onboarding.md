...
## Services used
- [auth_service.dart](../../lib/src/services/auth_service.dart) — wraps `FirebaseAuth`, holds `justSignedUp` flag so the router knows to push to onboarding after sign-up. Exposes `discardPendingSignup` which deletes the user doc + auth user (timeouts: doc 6 s, auth 8 s). `signUp` returns a `SignupResult { user, emailSendError }` so the caller can tell whether the verification email actually sent — failures are logged (`debugPrint`) and mapped to a user-readable string via `_verificationSendError` (handles `too-many-requests`, `network-request-failed`, `user-not-found`/`no-current-user`, `invalid-recipient-email`/`invalid-email`, with a fallback that includes the raw code).
...
- Verification-email send is best-effort: `signUp` keeps the account alive even if the mail send throws, returning the error inside `SignupResult.emailSendError`. The signup screen passes that into `/otp` as a `sendError` query param so the OTP screen's error banner is pre-populated — before this, the failure was caught with `catch (_)` and the user had no idea why their inbox stayed empty.
